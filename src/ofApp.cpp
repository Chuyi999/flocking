...
void agent::update(agent *arr){  // influence the main vector 
  ...
  */
   ...
   ofVec2f mouseAngle = calcCohesion(arr, MAX_AGENTS);
    if(mouse !=noVec){
  ...
  if (tooClose != noVec) {
    ...
    if (velDir.angle((tooClose - location)) < 0) 			// if it is on our left side
      ...
      tooCloseDelta = velDir.angle((tooClose - location).rotate(-90));  // move opposite to mass
    velDir.rotate(tooCloseDelta*.13);					// apply centroid gravity (weighted)
  }
  
    location += velDir*2;	// move to current location
    //location += mouse;
}
...
void agent::draw(){
  ...
  ofVec2f p1(ofRandom(20),  ofRandom(12));	// triangle pointing up w middle at origin
  ofVec2f p2(0, -12);
  ofVec2f p3(-6.25,  12);
  ...
  ofDrawTriangle(p1, p2, p3);
    if ( mouse.y<ofGetHeight()/2){
       ofDrawTriangle(p1/2, p2/2, p3/2);
    }
    else{
        ofDrawTriangle(p1*2, p2*2, p3*2);
    }
...
void ofApp::draw(){
  ...
  for (int i=0; i < MAX_AGENTS; i++) {
    ofSetColor(255,ofRandom(100,200),ofRandom(20,50));
    Agents[i].draw();
  }
}
...
void ofApp::mousePressed(int x, int y, int button){
    Agents[curAgent].start(ofRandom(x*1.5), ofRandom(y*1.5));
    curAgent++;
